...
#   infra/grant-access.sh livya@sahrdaya.ac.in
#   infra/grant-access.sh group:dev-team@sahrdaya.ac.in
#   infra/grant-access.sh domain:sahrdaya.ac.in
set -euo pipefail
cd "$(dirname "$0")/.."
source infra/config.sh
[ $# -ge 1 ] || { echo "usage: $0 <email | user:email | group:email | domain:example.com> ..."; exit 1; }
for m in "$@"; do
