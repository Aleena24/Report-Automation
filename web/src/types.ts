...
export interface FieldMeta { key: string; label: string; help?: string; group: string; type?: 'boolean' | 'number' | 'select' | 'textarea'; options?: string[] }
export interface SettingsResponse {
  config: Record<string, unknown>; fields: FieldMeta[]; readOnly: boolean;
  meta: { serviceAccountEmail: string; projectId: string; region: string; authMode: string; transport: string; smtpConfigured: boolean; appUrl: string; schedule: Array<{ job: string; time: string; note: string }> };
}
