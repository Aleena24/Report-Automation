 ...
 *   node dist/cli.js auto        morning batch before 12:00 IST, evening digest after (what the scheduler runs)
 ...
 *   node dist/cli.js preview me@sahrdaya.ac.in   every mail, to that address only
 */
...
import { timeIST } from './lib/dates.js';
...
async function main() {
  ...
  let mode = cmd;
  if (cmd === 'auto') mode = timeIST(deps.now()).hour < 12 ? 'morning' : 'evening';
